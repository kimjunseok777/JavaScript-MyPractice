...
/*
논리연산자 : 참이냐 거짓이냐를 따지는 연산자
    true / false
    ex)
    let a = 5;
    let b = 3;
    console.log(a > b) // true (초과일 때)
    console.log(a >= b) // true (이상일 때) : a 가 b 보다 크거나 같다 (이상이다) 라는 뜻

    참, 거짓을 판단할 수 있는 연산이 조건에 들어왔을 때 -> 이것을 "논리연산자" 라고 한다
    true / false 를 출력
    대부분 "비교연산자" 와 같이 쓰임
...
비교연산자 :
    ...
    ===  일치비교 : "값" 과 "데이터 유형" 모두 비교한다 -> '확실한 비교'
        --> 동등비교보다 "일치비교" 가 더 많이 쓰인다  "==="
        --> 부등비교보다 "불일치비교" 가 더 많이 쓰인다  "!=="

        ex)
            a = 3;  (숫자)
            b = "3";  (문자) (쌍 따움표 쓰임)
            a == b (true)  --> 값만 보기 때문에 'true' 가 나옴
            a === b (false)  -->  값과 데이터 유형 모두 보기 때문에 'false' 가 나옴

-----------------------------------------------------------------------------------------------

    !=  부등비교 : "값만"  -->  "값" 이 안같아?
    !==  불일치비교 : "값과 데이터 유형"  -->  "값" 과 "데이터 유형" 이 안같아?
        ex)
            a = 3;
            b = "3";
...
console.log(a) // 6
console.log(++a) // 7
console.log(a++) // 7  ++가 뒤어오면 마지막에 더하는 것 (출력을 하고 나서 더하는 것)
console.log(a) // 8
...
0 === -0 // true  -> 0은 양의 정수와 음의 정수에 포함되지 않기에 true 이다
...
/*
    ...
    논리곱 연산자 :  &&  -->  n개 다 true 야?  ->  모든 조건을 만족하는지 물어보는 것 (모든 조건이 true여야함)
    부정 연산자 :  !  -->  반대야?
*/
...
// c 가 1과 4 이상이면 콘솔로그 실행
// 이런식으로 많이 쓰임

if(c  > 1 && c > 7) {
    console.log(c) // 실행하지 않음 // false 로 실행 X
}
// c 가 1과 7 이상이면 콘솔로그 실행 / BUT c 는 7 보다 작기 때문에 실행하지 않는다
